...
async function realizarLogin(req, res) {
    try {
        // Captura os dados vindos do corpo da requisição (formulário)
        const { email, senha } = req.body;

        // Busca o usuárioo no banco de dados com base no e-mail e senha informados na tela de Login
        const [usuarios] = await conexao.query(
            'SELECT * FROM usuario WHERE email = ? AND senha = ?',
            [email, senha]
        );

        // Se não encontrar usuárioo, redireciona a página de login com a mensagem de erro
